document.addEventListener("DOMContentLoaded", function () {
  fetch("http://localhost:8080/api/bitcoin-history")
    .then((response) => response.json())
    .then((data) => {
      const formattedData = rowConverter(data);
      drawCandleSimple(
        "simpleCandleChart1",
        formattedData,
      ...
      drawtwoline("linetwocolumn2", formattedData, "Detailed Analysis");
      drawtwoline1(
        "linetwocolumn4",
        formattedData.date,
        formattedData.close,
...
function rowConverter(data) {
  ...
  data.forEach((d) => {
    ...
    high.push(d.high);
    low.push(d.low);
    adj_Close.push(d.adjClose);
    volume.push(d.volume);
    close.push(d.close);
  });

  return { date, open, high, low, adj_Close, volume, close };
}
